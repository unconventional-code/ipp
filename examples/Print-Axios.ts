...
// const buffer = printer.executePrintJob(
// 	"Print-Job",
//   {
...
async function main() {
	const createdJobResponse = await axios.post(url,
	...
	const buffer = printer.executeSendDocument(
			"Send-Document",
			{
				"operation-attributes-tag": {
	...
	const sendDocumentResponse = await axios.post(url, buffer, {
		...
		const sendData = printer.decodeMessage(Buffer.from(sendDocumentResponse.data));
